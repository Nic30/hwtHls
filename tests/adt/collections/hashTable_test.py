...
class HashTable_TC(BaseSsaTC):
    ...
    def test_SWAP_BY_INDEX(self):
        dut = HashTable()
        dut.ITEMS_PER_TABLE = 8
        dut.CLK_FREQ = int(100e6)
        self.compileSimAndStart(dut, target_platform=Artix7Slow())
        ...
        dut.cmd._ag.data.extend(self._cmd(HASH_TABLE_CMD.SWAP_BY_INDEX, 1, key, value, key)  for key, value in KEY_VALUE_PAIRS)
        
        self.runSim(int((len(KEY_VALUE_PAIRS) * 2 + 4) * freq_to_period(dut.CLK_FREQ)))
        ...
        self.assertValSequenceEqual(dut.cmdRes._ag.data, cmdResRef)
        mem = self.rtl_simulator.model.tableRam_inst.io.ram_memory.val.val

        for key in sorted(memRef.keys()):
            item = mem[key]
            value = memRef[key]
            itemRef = dut.item_t.from_py({
                "itemValid": 1,
                "key": key,
                "value": value,
            })._reinterpret_cast(dut.item_flat_t)

...
class BreakHandshakeCycles_TC(BaseSerializationTC):
    ...
    def _test_graph(self, graph: List[Tuple[str, List[List[HlsNetNode]]]]):
        ...
        for elmName, elmNodeTemplate in graph:
            ...
            elm = ArchElementPipeline(netlist, elmName, elmName + "_", stages=stages)
    ...
    def test_wire_2parallel_2b(self):
        graph = [("p0", [["i0", "o0", 'wb0', 'wb1']]),
                 ("p1", [["i1", "o1", 'rb0 init:1', 'rb1 init:1']])]
        self._test_graph(graph)

    def test_wire_2parallel_1f1b(self):
        graph = [("p0", [["i0", "o0", 'wf0', 'wb1']]),
                 ("p1", [["i1", "o1", 'rf0 init:1', 'rb1 init:1']])]
        self._test_graph(graph)
    ...
    def test_loop_1b(self):
        graph = [("p0", [["i0", "o0", "rb0 init:1", "wb0"]])]
        self._test_graph(graph)

    def test_loop_1b1f(self):
        graph = [("p0", [["i0", "o0", "rb0 init:1", "wb0", "wf1", "rf1"]])]
        self._test_graph(graph)
    ...
    def test_loop_prequel(self):
        graph = [("p0", [["i0", "wf0"]]),
                 ("loop0", [["o0", "rf0", "rb1 init:1", "wb1"]])]
        self._test_graph(graph)

    def test_loop_prequelOptional(self):
        graph = [("p0", [["i0", "wf0"]]),
                 ("loop0", [["o0", "rf0 ec sw", "rb1 ec sw init:1", "wb1"]])]
        self._test_graph(graph)

    def test_loop_2clk_1b(self):
        graph = [("p0", [["i0", "o0", "rb0 init:1", "wf1"],
                         ["rf1", "wb0"]])]
        self._test_graph(graph)

    def test_loop_2clk_1b_init1(self):
        graph = [("p0", [["i0", "o0", "rb0", "wf1"],
                         ["rf1 init:1", "wb0"]])]
        self._test_graph(graph)

    # def test_loop_2clk_1b_noInit(self):
    #     graph = [("p0", [["i0", "o0", "rb0", "wf1"],
    #                      ["rf1", "wb0"]])]
    #     with self.assertRaises(ChannelDeadlockError):
    #         self._test_graph(graph)
    ...
    # def test_loop_2clk_1b_wsw_noInit(self):
    #     graph = [("p0", [["i0", "o0", "rb0", "wf1"],
    #                      ["rf1", "wb0 sw"]])]
    #     with self.assertRaises(ChannelDeadlockError):
    #         self._test_graph(graph)
    ...
    def test_loop_2clk_1b_rsw_init1(self):
        graph = [("p0", [["i0", "o0", "rb0 sw", "wf1"], ["rf1 init:1", "wb0"]])]
        self._test_graph(graph)

    def test_loop_2clk_1b_rwsw_init1(self):
        graph = [("p0", [["i0", "o0", "rb0 sw", "wf1"], ["rf1 init:1", "wb0 sw"]])]
        self._test_graph(graph)

    def test_loop_2clk_1b_init2(self):
        graph = [("p0", [["i0", "o0", "rb0 init:1", "wf1"], ["rf1 init:2", "wb0"]])]
        self._test_graph(graph)

    def test_loop_2b(self):
        graph = [("p0", [["i0", "o0", "rb0 init:1", "wb0", "rb1 init:1", "wb1"]])]
        self._test_graph(graph)

    def test_loop_2parallel(self):
        graph = [("p0", [["i0", "rb0 init:1", "wb0"]]),
                 ("p1", [["rb1 init:1", "wb1", "o0"]])
                 ]
        self._test_graph(graph)

    def test_loop_3parallel_1f(self):
        graph = [("p0", [["i0", "rb1 init:1", "wb1", "wf2"]]),
                 ("p1", [["rf2", "rb3 init:1", "wb3", "wf4"]]),
                 ("p2", [["rf4", "rb5 init:1", "wb5", "o0"]])]
        self._test_graph(graph)

//===- CodeExtractor.cpp llvm-21.1.2 - Pull code region into a new function -----------===//
//
...
#define DEBUG_TYPE "hwtHls-code-extractor"
...
static cl::opt<bool>
AggregateArgsOpt("hwtHls-aggregate-extracted-args", cl::Hidden,
...
void CodeExtractor::constructFunctionDeclaration_inheritAttributes(Function *oldFunction,
...
void CodeExtractor::constructFunctionDeclaration_updateNewEntryFreq(BlockFrequency EntryFreq,
...
void CodeExtractor::extractCodeRegion_discardIncompatibleAssumes() {
...
BlockFrequency CodeExtractor::extractCodeRegion_calculateEntryFreq(BasicBlock* header, DenseMap<BasicBlock *, BlockFrequency>& ExitWeights) {
...
CodeExtractor::extractCodeRegion(const CodeExtractorAnalysisCache &CEAC,
  ...
  extractCodeRegion_discardIncompatibleAssumes();
  ...
  BlockFrequency EntryFreq = extractCodeRegion_calculateEntryFreq(header, ExitWeights);
...
void CodeExtractor::_emitFunctionBody_replaceUsesOfInputs(const ValueSet &inputs, const SmallVectorImpl<Value *> &NewValues) {
	assert(inputs.size() == NewValues.size());
...
void CodeExtractor::_emitFunctionBody_sink(const ValueSet &SinkingCands, BasicBlock * newFuncRoot) {
...
void CodeExtractor::_emitFunctionBody_return(Function * newFunction,
		BasicBlock * header, BasicBlock * newFuncRoot) {
...
void CodeExtractor::_emitReplacerCall_constructSwitch(Function * newFunction,
		BasicBlock* codeReplacer, CallInst * call) {
  LLVMContext & Context = newFunction->getContext();
...
CallInst *CodeExtractor::emitReplacerCall(
  ...
  _emitReplacerCall_constructSwitch(newFunction,
  		codeReplacer, call);

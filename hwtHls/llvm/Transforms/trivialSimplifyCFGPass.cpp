...
bool tryRemoveSingleSuccessorSinglePredecessorBlock(BasicBlock *BB, llvm::SmallSetVector<BasicBlock*, 16>& WorkList) {
	// Remove empty basic block if has single successor and predecessor and may be replaced by predecessor in successor PHIs
	if (BB->begin() == BB->end() || &*BB->begin() == BB->getTerminator() || ++BB->begin() == BB->end()) {
		if (auto *SucBB = BB->getSingleSuccessor()) {
			...
			if (auto *PredBB = BB->getSinglePredecessor()) {
				...
				// update PHIs
				for (PHINode &SucPhi : SucBB->phis()) {
					if (std::find(alreadyHasTheValueInPhis.begin(), alreadyHasTheValueInPhis.end(), &SucPhi)
...
bool trySimplifyTerminator(BasicBlock & BB, llvm::SmallSetVector<BasicBlock*, 16> & worklist) {

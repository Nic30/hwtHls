...
static LoopUnrollResult tryToUnrollStreamSegmentLoop(
	...
	auto streamArgI = getOptionalIntHwtHlsLoopAttribute(
		&L, StreamSegmentLoopUnrollPass::METADATA_NAME_io);
	auto allowSoFOnlyFor = getOptionalIntVecHwtHlsLoopAttribute(
		&L, StreamSegmentLoopUnrollPass::METADATA_NAME_allowSoFOnlyFor);
	if (!streamArgI.has_value()) {
		LLVM_DEBUG(
			dbgs() << "  Loop does not have hwthls.loop.streamloopunroll.io "
	...
	std::string errTmp =
		"StreamSegmentLoopUnrollPass requires all accesses to IO to be present "
		"inside of the loop, users outside of loop:\n";
	...
	// writeCFGToDotFile(F, "tmp/StreamSegmentLoopUnrollPass.1.normalized.dot",
	// AMForDebug, 		false, true);
	...
	// :note: unique_ptr is used because ValueToValueMapTy is non-copyiable,
	// non-movable
	auto valueMaps = std::unique_ptr<ValueToValueMapTy[]>(
		new ValueToValueMapTy[streamProps.segmentCnt]); // last value map is has
														// key=value and is for
														// original code

	copyCodeForLanes(streamProps, LI, L, DTU, BBs, loopBodyCopies, valueMaps,
					 F);
	DTU.flush();
	...
	// * entry block of non-(last for input, first for output) lane will have no
	// predecessor
	//   * :note: for input the section starts after LoadInst and thus the lane
	//   for last segment will be connected
	//            before first lane
	// * exit block of non-last lane will have no successors
	//   * :note: jumps between lanes are not constructed yet and this is
	//   original exit/backedge of the loop

	// convert registers defined inside of the loop to tmp variable using
	// AllocaInst if its liveness is crossing the newly created edges or if they
	// are primary inputs/outputs of the section which will be unrolled :note:
	// Tmp variables are used because llvm::RemapInstruction would not be
	// sufficient
	//        because block in section of next lane does not need to be
	//        dominated by a single predecessor in previous lane, thus some
	//        extra PHIs are required if this is the case The llvm CodeExtractor
	//        approach (liveins/liveouts of extracted region) is not applicable
	//        there because it works only for regions dominated by a single
	//        block there new section are generated from code between
	//        LoadInst/Store inst which may not dominate each other and blocks
	//        from different lanes will be connected together. Resulting in a
	//        situation where variable which was originally provided from this
	//        lane may be provided from different one because there was some
	//        additional Load/Store on the path. Which is now processed by a
	//        different lane.

	// if this IO is input extend reads in the first lane to bus word width and
	// in all other use this value instead if this IO is output, extend writes
	// in last lane to bus word width and all others will stack data to tmp
	// variable instead

	// reroute edges which were generated after original LoadInst/StoreInst of
	// this IO
	...
	rerouteLaneCfgAndSegmentValue(Builder, F, DTU, LI, streamProps, tmpAllocas,
								  IoInstructions, valueMaps, BBToLaneIndex,
								  BBToIndexInloopBodyCopies, loopBodyCopies,
								  allowSoFOnlyFor);
	// F.dump();
	// writeCFGToDotFile(F, "tmp/StreamSegmentLoopUnrollPass.2.dot", AMForDebug,
	//		false, true);

	// this generates code for lanes in loop, but it is expected that the top
...
StreamSegmentLoopUnrollPass::run(llvm::Function &F,
	...
	if (auto *LAMProxy =
			AM.getCachedResult<LoopAnalysisManagerFunctionProxy>(F))
	...
	while (!Worklist.empty()) {
		...
		// has a huge working set size, in which case we disable peeling to
		// avoid bloating it further.
		...
		// UnrollOpts).
		LoopUnrollResult Result = tryToUnrollStreamSegmentLoop(
			F, L, DT, LI, SE, TTI, AC, ORE, BFI, PSI,
	...
	if (!Changed)
		return PreservedAnalyses::all();
	// writeCFGToDotFile(F,
	// "tmp/StreamSegmentLoopUnrollPass.3.beforeReg2Mem.dot", AM, 			false, false);

...
void sinkMergableSequenceOfWritesMoveWrites(
	llvm::Loop &L, llvm::LoopInfo &LI, llvm::ScalarEvolution &SE,
	llvm::DominatorTree &DT, llvm::DomTreeUpdater &DTU,
	...
	DTU.flush();
	// prepare preheader for tmp variables which will be used for write data and
	// conditions
	...
	for (const OptionalStreamWriteCFGFragment &wr : mergableWriteSeqeunce) {
		...
		auto wrBBTerm = SplitBlockAndInsertIfThen(
			enCond,										   /*SplitBefore*/
...
void sinkMergableSequenceOfWrites(
	llvm::Loop &L, llvm::LoopInfo &LI, llvm::ScalarEvolution &SE,
	llvm::DominatorTree &DT, llvm::AssumptionCache &AC,
	const llvm::TargetTransformInfo &TTI, bool PreserveLCSSA,
	const SmallVector<OptionalStreamWriteCFGFragment> &mergableWriteSeqeunce) {
	// edges from exit blocks are likely exit from loop and continue in write
	// sequence this means that there is no post dominating block
	//
	// :note: the goal is to move all writes behind last "exit" block with the
	// lowest amount of code replication possible
	//   specially without replication of writes
	//   However if the exit block has more than 1 successor some replication is
	//   necessary because write has to be performed on every path.

	// 1. dedicated exit blocks (should be prepared by formDedicatedExitBlocks
	// and LoopSimplifyNormalForm)
	//   (each block which is outside of the loop and is targeted from some
	//   block from this loop has all predecessors in this loop)
	//    single backedge
	formLCSSA(L, DT, &LI,
			  &SE); // because we need to known all liveins for hub at the end

	// 2. create a tmp variable for each write which will be assigned 0 at
	// header and 1 on original location of a write
	...
	// any variable alive on backedge or exit needs to have phi which select the
	// value because ControlFlowHub is shared block mixes exit and backedge path
	// and SSA requires def before use
	...
	exit0 =
		splitBlockBefore(exit0, exit0->getFirstNonPHIIt(), &DTU, &LI, /*MSSAU*/
						 nullptr, exit0->getName() + ".exit1");

	BasicBlock *writesEntry =
		SplitEdge(exit0, exit1, &DT, &LI, /*MSSAU*/
				  nullptr, latch0->getName() + ".writesEntry");
	BasicBlock *writesExit =
		SplitEdge(writesEntry, exit1, &DT, &LI, /*MSSAU*/
	...
	auto *loopExitCond =
		Builder.CreatePHI(Builder.getInt1Ty(), 0, "loopExitCond");
	...
	// exit and after blocks were originally in parent loop now they are in L
	// because backedge leads from latch1 which is now after writesExit
	LI.changeLoopFor(exit0, &L);
	LI.changeLoopFor(writesEntry, &L);
	LI.changeLoopFor(writesExit, &L);

	DTU.applyUpdates({
		{DominatorTree::Delete, latch0, latch1},	  //
	...
	sinkMergableSequenceOfWritesMoveWrites(L, LI, SE, DT, DTU, AC, TTI,
										   ...
										   Builder, writesEntry, writesExit);

	// llvm_unreachable("NotImplemented sinkMergableSequenceOfWrites");
}

/**
 * this loop was unrolled to satisfy throughput, that means that there are
 likely
 ...
 *   and to write it to IO, on the other hand if writes are sinked they may be
 merged
 *   which will then drastically reduce number of possible offsets for each
 write byte
 ...
 *
 *  As write with side-effect is between reads with side-effect, this is
 generally non-optimizable pattern
...
void sinkStreamWritesInLoop(llvm::Loop &L, llvm::ScalarEvolution &SE,
	...
	for (auto *BB : L.blocks()) {
		llvm::CallInst *wr =
			HwtHlsSimplifyCFGPass_streamWriteMergeInSingleBlock(Builder, *BB,
																SQ);
		if (wr) {

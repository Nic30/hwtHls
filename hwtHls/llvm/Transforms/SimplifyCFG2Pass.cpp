...
class SimplifyCFGOpt2 {
  ...
  Value *isValueEqualityComparison(Instruction *TI, bool checkParentPredecessors);
  BasicBlock *GetValueEqualityComparisonCases(
      Instruction *TI, std::vector<ValueEqualityComparisonCase> &Cases);
  bool FoldValueComparisonIntoPredecessors(Instruction *TI, IRBuilder<> &Builder);
...
public:
  SimplifyCFGOpt2(DomTreeUpdater *DTU,
                 const DataLayout &DL,
...
Value *SimplifyCFGOpt2::isValueEqualityComparison(Instruction *TI, bool checkParentPredecessors) {
  Value *CV = nullptr;
  if (SwitchInst *SI = dyn_cast<SwitchInst>(TI)) {
    // Do not permit merging of large switch instructions into their
    // predecessors unless there is only one predecessor.
    if (!checkParentPredecessors || !SI->getParent()->hasNPredecessorsOrMore(128 / SI->getNumSuccessors()))
      CV = SI->getCondition();
  } else if (BranchInst *BI = dyn_cast<BranchInst>(TI))
...
llvm::PreservedAnalyses SimplifyCFGPass2::run(llvm::Function &F, llvm::FunctionAnalysisManager &AM) {
	...
	for (;;) {
		...
		for (Function::iterator BBIt = F.begin(); BBIt != F.end();) {
			...
			_changed |= opt.run(&BB);
			_changed |= SimplifyCFGPass2_normalizeLookupTableIndex(BB);
		}
		changed |= _changed;
		if (!_changed)
			break;

...
/// Fill a new leaf block for the binary lookup tree. It checks if the
...
void NewLeafBlock(BasicBlock * NewLeaf, CaseRange &Leaf, Value *Val, ConstantInt *LowerBound,
...
BasicBlock *SwitchConvert(CaseItr Begin, CaseItr End, ConstantInt *LowerBound,
  ...
  SmallVector<BasicBlock*> leafBlocks; // (blocks with case condition chained in sequence of cond. br)
  for (unsigned i = 0; i < Size; ++i) {
	  BasicBlock *NewLeaf = BasicBlock::Create(Val->getContext(), "LeafBlock");
	  F->insert(++OrigBlock->getIterator(), NewLeaf);
	  leafBlocks.push_back(NewLeaf);
  }
  ...
  if (Size == 1) {
     ...
     NewLeafBlock(leafBlocks[0], *Begin, Val, LowerBound, UpperBound, OrigBlock,
                         Default);
	 return leafBlocks[0];					 
  }
  
  BasicBlock *elseBB = Default;
  for (auto [C, leafBB]: reverse(zip(make_range(Begin, End), leafBlocks))) {
    NewLeafBlock(leafBB, C, Val, C.Low, C.High, OrigBlock,
                        elseBB);
	elseBB = leafBB;
  }
...
PreservedAnalyses LowerSwitchWithIfChain::run(Function &F, FunctionAnalysisManager &AM) {

...
bool tryHoistFromCheapBlocksWithcSwitchLikeCmpBr(llvm::BranchInst *BI,
	...
	if (isValueEqualityComparation(BI->getCondition(), brInfo.predicate,
		...
		BasicBlock &BBExit = *branchInfo[0].TSucc; // common exit for all block in the chain
		BasicBlock &BBDefault = *branchInfo.back().FSucc; // a block on the end of the chain
		...
		for (auto &BrInfo : branchInfo) {
			...
			// replace conditional jump depending on cmp with condition of new switch
			// with jump to common BBExit
			...
			if (DTU) {
				...
				DTU->applyUpdates( { { DominatorTree::Insert, BBTop, Dest }, //
						{ DominatorTree::Delete, Dest, BrInfo.FSucc }, // :note: Dest->ExitBB remains
						});
			}
		}
		// BBDefault is now entered from BBTop instead of last case BB in branchInfo

...
inline bool _DBG_SimplifyCFGOpt2_check(bool changed, Function &F,
		const char *file, int line, const char *func, const char *cmdStr) {
#ifdef DBG_VERIFY_AFTER_EVERY_MODIFICATION
	if (changed) {
		if (verifyFunction(F, &errs()))
			__assert_fail(cmdStr, file, line, func);
	}
#endif
	return changed;
}
		...
		if (CE->getOpcode() == Instruction::IntToPtr)
			if (ConstantInt *CI = dyn_cast<ConstantInt>(CE->getOperand(0))) {
				...
				if (CI->getType() == PtrTy)
					...
					return cast<ConstantInt>(ConstantFoldIntegerCast(CI, PtrTy, /*isSigned=*/
					false, DL));
			}
	return nullptr;
}
				...
				if (BranchInst *BI = dyn_cast<BranchInst>(T)) {
					...
					if (BI->isUnconditional())
						...
						if (BI->getSuccessor(0) == BB)
							Assumption = Builder.CreateAssumption(
									Builder.CreateNot(Cond));
						else
							Assumption = Builder.CreateAssumption(Cond);
						if (AC)
							AC->registerAssumption(
									cast<AssumeInst>(Assumption));
						Builder.CreateBr(
...
static bool IncomingValuesAreCompatible(BasicBlock *BB,
	...
	return all_of(BB->phis(), [IncomingBlocks, EquivalenceSet](PHINode &PN) {
		...
		if (IV0 == IV1)
			return true;
		if (EquivalenceSet && EquivalenceSet->contains(IV0) && EquivalenceSet->contains(IV1))
	return true;
		return false;
	});
}
		...
		for (BasicBlock *Pred : predecessors(BI->getParent()))
			if (BranchInst *PBI = dyn_cast<BranchInst>(Pred->getTerminator()))
				if (PBI != BI && PBI->isConditional())
					if (DBG_SimplifyCFGOpt2_check(hwtHls::SimplifyCondBranchToCondBranch(PBI, BI, DTU, DL,
							TTI)))
						return requestResimplify();
...
bool SimplifyCFGOpt2::simplifyOnce(BasicBlock *BB, bool &exprChanged) {
	...
	case Instruction::Switch:
		Changed |=
				simplifySwitch(cast<SwitchInst>(Terminator), Builder, exprChanged);
		break;
	case Instruction::Br:

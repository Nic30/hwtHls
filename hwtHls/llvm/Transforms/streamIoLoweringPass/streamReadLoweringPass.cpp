...
public:
	...
	//
	// Variant 0(PWF_STATIC): in this case the chunk is always aligned to start
	// of the word
	//    word[0]
	//    word[1]
	//    ...
	...
	//    * The chunk have same number of words for every alignment but some
	//    offset >0
	...
	//   There is offset=0 which does not require leftover word and some offset
	//   which end overlaps to next next word at end. word[-2] preLastWord
	//   (leftover) word[-1] lastWord (optional input word read to cover for
	//   offsets which require 1 more word
	...
	enum PartialWordsFormat {
		...
		// :note: leftover word is there if some offset != 0 and thus data from
		// previous word have to be used
	...
	StreamReadChunk(const StreamChannelProps &streamProps,
		...
		wordCntRange(streamProps._resolveMinMaxSegmentCount(possibleOffsets,
															chunkWidth)), //
				...
				(possibleOffsets[0] == 0 &&
				 all_of(possibleOffsets,
						[&streamProps, chunkWidth](size_t off) {
							return off + chunkWidth <= streamProps.dataWidth;
						}))) {
				// consumes extra word for some offset or
				// always fits into 1 word but it may be leftover or newly
				// loaded word
	...
	bool mayReadLeftoverData() const {
		// first word may be read of leftover word (and may also be read of new
		// bus word)
	...
	bool readsLeftoverDataWithOffset(size_t off) const { return off != 0; }
	...
	size_t getNumOfSkippedWordsForOffset(size_t off) const {
		...
		case PartialWordsFormat::PWF_LEFTOVER_OPTIONAL_STATIC: {
			...
			if (readsLeftoverDataWithOffset(off)) {
				if (wCnt == wordCntRange.first) {
					...
					//  |   | 0 | 1 | // leftover
					//  |   | 0 | 1 | // optional word containing leftover
					//  because it was not loaded | 2 | 3 | 4 | // 1st static
					//  word
					...
					//  |   |   | 0 | // leftover
					//  | 1 | 2 | 3 | // optional word containing newly loaded
					//  data | 4 |   |   | // 1st static word
					return 0;
				} else {
					llvm_unreachable(
				...
				if (wCnt == wordCntRange.first) {
					...
					//  |   |   |   | // leftover
					//  | 0 | 1 | 2 | // optional word containing newly loaded
					//  data | 3 | 4 |   | // 1st static word skip the leftover
					//  word
					return 1;
				} else {
					llvm_unreachable(
		...
		llvm_unreachable("StreamReadChunk::getNumOfSkippedWordsForOffset: "
		...
		//		// skip read of optional word at the beginning because leftover
		// word is used
		//		// instead of it
		//		++skipped;
		//	}
		...
		//	if (*off == 0) {
		//		// (Variant 1, 2)
		//		// now not reading last word loaded for predecessor but other
		// offsets variant are using it
		//		++chunkWordIt;
		//		++skippedCnt;
		//	}
		// }
		...
		//				== wordCntRange.first) {
		//	// (Variant 2)
		//	// some other offset is so large that there is 1 additional word on
		// end required
...
protected:
	...
	// create loads from IO which will always happen for every combination of
	// offset
	void _prepareStaticPrevWordVars(StreamReadChunk &readInfo,
									size_t staticWordCnt,
									bool skipDisabledSegments);
...
void StreamReadRewriter::_handleOptionalReadsDependingOnCurrentOffset(
	...
	for (size_t off : possibleOffsets) {
		...
		if (off == 0 || wCnt > wordCntRange.first) {
			...
			auto *offEn = Builder.CreateICmpEQ(
	...
	Instruction *thenBlockTerm = llvm::SplitBlockAndInsertIfThen(
		extraReadEn, read, false, /*BranchWeights*/ nullptr, DTU, LI);
	Builder.SetInsertPoint(thenBlockTerm);
	auto *ioWordLd =
		Builder.CreateLoad(streamProps.segmentTy, streamProps.ioArg,
						   /*isVolatile*/ true, read->getName() + ".optLd");
...
void StreamReadRewriter::_prepareStaticPrevWordVars(StreamReadChunk &readInfo,
	...
	// we obtain the max number of bus words for any offset, if some offset
	// variant requires +1 word, it is already loaded in if true the read data
	// is expected to always be present and well formated and no additional
	// check is required

	// :note: preLastWord/lastWord are provided externally because its read is
	// conditional on offset
	//   and this function construct only minimal number of reads shared for all
	//   variants
	assert((readInfo.wordCntRange.first == readInfo.wordCntRange.second ||
			readInfo.wordCntRange.first + 1 == readInfo.wordCntRange.second) &&
		   "max and min number of words for this chunk can be only "
		   "(chunkWidth//dataWidth)(+1)");
	...
	// :note: if this is the case which can read +1 words the load is already
	// prepared before
	...
	for (size_t i = 0; i < staticWordCnt; ++i) {
		...
		if (!readInfo.isReliable() && readInfo.inWords.size() &&
			(i > 1 || readInfo.partialWordFormat !=
						  StreamReadChunk::PartialWordsFormat::PWF_STATIC)) {
			// construct if-then for next read only if previous did not end with
			// eof :note: original read instruction will be after this new
			// if-then
			...
			Instruction *thenBlockTerm = llvm::SplitBlockAndInsertIfElse(
				prevEoF, readInfo.read, false, /*BranchWeights*/ nullptr, DTU,
				LI);
			BB = readInfo.read->getParent();
			assert(BB == thenBlockTerm->getSuccessor(0));
			Builder.SetInsertPoint(thenBlockTerm);
		}
		auto *ioWordLd = Builder.CreateLoad(
			streamProps.segmentTy, streamProps.ioArg, /*isVolatile*/ true,
	...
	if (readInfo.isReliable()) {
		...
		if (eofCheckingBlocks.size()) {
			// the values in readInfo.inWords are localized in sequence of
			// if-then, we have to create phis to select PoisonValue if if-then
			// was not performed BB0 ->  BB1 -> BB2 -------+
			//  |       |      |         |
			// eof0    eof1   eof2       v
			//  +-------+------+------> BB with original read
			auto inWordBegin = readInfo.inWords.begin() +
							   readInfo.inWords.size() -
							   eofCheckingBlocks.size();
			...
			for (auto &w : make_range(inWordBegin, readInfo.inWords.end())) {
				// for each out word construct phis for every member of
				// StreamChannelWordValue the value will be PoisonValue/0 if
				// value have not been yet read
			...
			// :note: items in readInfo.inWords now using phis, original values
			// are backuped in loadedWords
			auto addPhiIncomingUnused = [](StreamChannelWordValue &w,
										   BasicBlock *newPredBB) {
				// sof/eof/enable/error are or-ed (using 0 as default)
				// data/mask is concatenated (using PoisonValue for data, 0 for
				// mask as default) empty is summed (using wordbytecnt as
				// default)
			...
			auto addPhiIncomingSelf = [](StreamChannelWordValue &wPhi,
										 StreamChannelWordValue &wIn,
										 BasicBlock *newPredBB) {
				for (const auto &[vPhi, vOriginal] :
					 zip(wPhi.asArray(), wIn.asArray())) {
					if (!vPhi)
						continue;
			...
			for (auto *eofCheckingBB : eofCheckingBlocks) {
				...
				for (const auto &[wPhi, wIn] :
					 zip(make_range(inWordBegin, readInfo.inWords.end()),
						 loadedWords)) {
					// in BB0 word 0 is undefined because it is read in then
					// block after BB0
				...
				++bbIndex;
				// sortPhiOperands(*thenBB->getUniqueSuccessor());
			}
		}
	}
}
...
void StreamReadRewriter::_consumeReadWordsAndCreateResultData(
	...
	for (BasicBlock *br : offsetBranches) {
		...
		// vector of parts to build replacement for value of this original ADT
		// read
		...
		for (auto chunkWordIt = prevWordVars.begin() + skippedCnt;
			...
			if (inWordOffset == 0) {
				...
				if (bitsToTake != DATA_WIDTH) {
					...
					if (readInfo.isReliable()) {
						// delete byte enable encoding as we assume all bytes
						// are valid because this is reliable read
						partRead.props =
							partRead.props.resize(partRead.props.dataWidth, {},
		...
		if (newOffset != 0) {
			...
			case ByteEnableEncoding::BEE_ENABLE_PLUS_EMPTY: {
				if (streamProps.hasEmpty()) {
					...
					size_t bytesInWord =
						streamProps.dataWidth / streamProps.byteWidth;
					size_t emptyLimit =
						bytesInWord - newOffset / streamProps.byteWidth;
		...
		// cover the case for unreliable reads on 1B io where io does not have
		// empty/mask but the read return has
		...
		llvm::Value *readRes =
			_readRes.flatten(Builder, isFollowedByMoreValidDataInLastWord);
		assert(readRes->getType()->getIntegerBitWidth() ==
			   readInfo.read->getType()->getIntegerBitWidth());
		if (!readRes->getName().contains(readInfo.read->getName())) {
			readRes->setName(readRes->getName() + "(" +
							 readInfo.read->getName() + ")");
		}
	...
	//  original read should always end up in sequel block because everything we
	//  generated should be before it
	Builder.SetInsertPoint(readInfo.read);
	//} else {
	...
	//}
	// the insertion point should be the place behind all newly generated
	// instructions which are implementing original stream read
	// pseudo-instruction
...
void StreamReadRewriter::_rewriteAdtAccessToWordAccessInstruction(
	...
	if (readIsMarker) {
		...
		if (isStart) {
			if (possibleOffsets.size() != 1) {
				if (possibleOffsets.empty()) {
					throw std::runtime_error("Can not find any offset of of "
											 "which the frame may start");
				} else {
					std::string err;
					std::stringstream ss(err);
					ss << "[todo] Use first word mask to resolve the "
						  "offsetVar, possibleOffsets: [";
					for (size_t off : possibleOffsets) {
						ss << off << ", ";
					}
					ss << "]";
		...
		if (possibleOffsets.size()) {
			// if number of words differs in offset variants we need to insert a
			// new block which is entered conditionally for specific offset
			// values :note: the information about which word is last is stored
			// in offset variable and does not need to be explicitly specified
			...
			case StreamReadChunk::PartialWordsFormat::
				PWF_LEFTOVER_OPTIONAL_STATIC: {
				auto leftoverWord = streamProps.getAllData(Builder);
				readInfo.inWords.push_back(leftoverWord);
				_handleOptionalReadsDependingOnCurrentOffset(
					possibleOffsets, readInfo.wordCntRange, chunkWidth, read);
				auto optionalWord = streamProps.getAllData(Builder);
				readInfo.inWords.push_back(optionalWord);
				if (readInfo.wordCntRange.second == 1)
					staticWordCnt = 0; // case where chunk always fits into 1 word and this
						   // word can be leftover or not
				else {
			...
			// * replace original read of ADT with a result composed of word
			// reads
...
StreamReadLoweringPass::run(llvm::Function &F,
	...
	IRBuilder<TargetFolder, IRBuilderCallbackInserter> Builder(
		...
		IRBuilderCallbackInserter([AC](Instruction *I) {
			if (AC) {
	...
	for (StreamChannelProps &s : streamProps) {
		...
		StreamIoDetector cfg(
			LVI, s.dataWidth, s.dataOffsetVar,

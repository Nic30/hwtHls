...
class HlsNetlistAnalysisPassMirToNetlist(HlsNetlistAnalysisPassMirToNetlistDatapath):
    ...
    * With code branches where which do not have a constant duration there is this problem:
    ...
    def _injectVldMaskToExpr(self, out: HlsNetNodeOut, maskAppliedTo: Set[HlsNetNodeOut], maskForOut: Dict[HlsNetNodeOut, Tuple[HlsNetNodeOut, HlsNetNodeOut]]) -> HlsNetNodeOut:
        """
        ...
        if isinstance(outObj, HlsNetNodeReadSync):
            ...
            if oldI not in maskAppliedTo:
                newI, maskToApply = self._injectVldMaskToExpr(outObj.dependsOn[0], maskAppliedTo, maskForOut)
                if newI is not oldI:
        ...
        for n in self.netlist.iterAllNodes():
            if isinstance(n, HlsNetNodeExplicitSync):
                ...
                if n.skipWhen is not None:
                    ...
                    _o, maskToApply = self._injectVldMaskToExpr(o, maskAppliedTo, maskForOut)

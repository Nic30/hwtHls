...
class SsaPassExtractPartDrivers():
    """
    ...
    def _collest_all_blocks(self, start: SsaBasicBlock, seen_blocks: Set[SsaBasicBlock]):
        seen_blocks.add(start)
        yield start
        for suc in start.successors.iter_blocks():
            if suc not in seen_blocks:
                yield from self._collest_all_blocks(suc, seen_blocks)
        ...
        if self._isConstantSlice(o) and (o in splitPointsOfVariable or
            ...
            for u in o.users:
                if u in splitPointsOfVariable or varEntirelyReplaced.get(u, False):
                    continue
                userIsIndex = isinstance(u, SsaInstr) and u.operator == AllOps.INDEX and isinstance(u.operands[1], HValue)
                if userIsIndex:
                    continue
                elif isinstance(u, SsaInstrBranch):
                    usedOnlyByReplaced = False
                    break
                else:
                    usedOnlyByReplaced = False
    ...
    def resolveFinalReplacedVarValue(self,
                                     ...
                                     varBitAlises: Dict[SsaValue, Optional[ConcatOfSlices]]):
            # print(v)
            if isinstance(v, HValue) or not varEntirelyReplaced.get(v, False):
                return v
            else:
                replacement = varBitAlises.get(v, None)
                if replacement is None:
                    # try:
                    #    assert not varEntirelyReplaced.get(v, False), v
                    # except:
                    #    raise
                    return v
                ...
                expanded = ConcatOfSlices(tuple(
                    self.resolveFinalReplacedVarValue(_v, (high, low), variableForRange, varEntirelyReplaced, varBitAlises)
                    for _v, high, low in replacement.slices
                ))
                # expanded = expanded.slice(*bitRange)
                if varEntirelyReplaced.get(v, False):
                    assert bitRange[0] - bitRange[1] == replacement.bit_length, (v, bitRange, replacement)
    ...
    def apply(self, to_ssa: AstToSsa):
        ...
        allBlocksSet = set()
        allBlocks = list(self._collest_all_blocks(to_ssa.start, allBlocksSet))

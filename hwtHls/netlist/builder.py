...
class HlsNetlistBuilder():
    ...
    It also implements some simple folding rules for common operators.
...
#        self._removedNodes: Set[HlsNetNode] = ObservableSet()
    ...
    def buildOp(self,
        ...
        if operator is HwtOps.NOT:
            # if operator is NOT automatically pre-compute not not x == x
    ...
    def buildOpManyDst(self,
        ...
        if name is not None and not name:
            ...
            name = None
        if res is not None:
            assert isinstance(res, HlsNetNodeOperator), ("HlsNetNodeOperator with multiple outputs should have its instance stored in operatorCache", res)
            if name is not None and len(res.obj._outputs) == 1 and res.obj.name is None:
                res.obj.name = name
        ...
        self.operatorCache[keyWithHConsts] = n
        return n
    ...
    def buildOpWithOpt(self, operator: HOperatorDef,
        ...
        elif operator == HwtOps.ADD:
            return self.buildAdd(*operands, operatorSpecialization=operatorSpecialization)
        elif operator == HwtOps.SUB:
            return self.buildSub(*operands, operatorSpecialization=operatorSpecialization)
    ...
    def buildAdd(self, a: HlsNetNodeOut, b: HlsNetNodeOut,
        ...
        if a is b:
            ...
            if w == 1:
                # always overflows or is 0
                return self.buildConst(BIT.from_py(0))

            # a+a = 2*a = a<<1
            return self.buildConcat(BIT.from_py(0), self.buildIndexConstSlice(a._dtype.__class__(w - 1), a, w, 1, operatorSpecialization, name))

        return self.buildOp(HwtOps.ADD, operatorSpecialization, a._dtype, a, b, name=name)

    def buildIncr(self, x: HlsNetNodeOut, en:Union[Literal[True], HlsNetNodeOut, HBitsConst]=True, saturateSigned=False, saturateUnsigned=False):
        t = x._dtype
        w = t.bit_length()
        if saturateSigned and w == 1:
            # can not increment because it would change sign
            return x
        xPlus1 = self.buildAdd(x, t.from_py(1))
        if saturateSigned:
            willNotOverflow = self.buildNe(x, t.from_py(mask(w - 1)))
        elif saturateUnsigned:
        ...
        if willNotOverflow is None:
            ...
            if isinstance(en, bool):
                ...
                willNotOverflow = self.buildAnd(willNotOverflow, en)
            return self.buildMux(t, (xPlus1, willNotOverflow, x))
    ...
    def buildSub(self, a: HlsNetNodeOut, b: HlsNetNodeOut,
        ...
        if a is b:
            # a-a = 0
            return self.buildConst(a._dtype.from_py(0))
    ...
    def buildAndVariadic(self, ops: Tuple[Union[HlsNetNodeOut, HConst], ...],
                         name:Optional[str]=None,
                         operatorSpecialization:OpSpecialization_t=None, opt=True):
    ...
    def buildAnd(self, a: Union[HlsNetNodeOut, HConst], b:Union[HlsNetNodeOut, HConst],
                 name:Optional[str]=None, operatorSpecialization:OpSpecialization_t=None, opt=True) -> HlsNetNodeOut:
    ...
    def buildAndOptional(self, a: Optional[Union[HlsNetNodeOut, HConst]], b: Optional[Union[HlsNetNodeOut, HConst]],
                         name:Optional[str]=None, operatorSpecialization:OpSpecialization_t=None, opt=True)\
            ->Union[HlsNetNodeOut, HConst, None]:
        if a is None:
            return b
        elif b is None or (opt and b is a):
            return a
        else:
            return self.buildAnd(a, b, name=name, operatorSpecialization=operatorSpecialization, opt=opt)
    ...
    def buildOrOptional(self, a: Optional[Union[HlsNetNodeOut, HConst]], b: Optional[Union[HlsNetNodeOut, HConst]],
        ...
        if a is None:
            ...
            if b is None or a is b:
                ...
                return self.buildOr(a, b, name=name, operatorSpecialization=operatorSpecialization, opt=opt)
    ...
    def buildIndexConst(self, a: HlsNetNodeOut, i: int,
        ...
        assert i >= 0, (a, width, i)
        assert i < width, (a, width, i)
    ...
    def buildIndexConstSlice(self, resT: HdlType, a: HlsNetNodeOut, high: int, low: Optional[int],
        ...
        if width == 1:
            ...
            assert resT == BIT, resT
            assert high <= width and high >= 0, (high, width)
            i = high
        elif low == high - 1:
            # selecting 1bit  x[high:low]
            i = high - 1
            assert resT.bit_length() == 1, resT
        else:
            assert high > low, (high, low)
            assert high <= width, (high, width)
            assert low >= 0, low
            i = slice(high, low, -1)
            assert resT.bit_length() == high - low, (resT, high, low)
    ...
    def buildShlConst(self, op0: HlsNetNodeOut, shAmount: int):
        ...
        return self.buildConcat(
            pad,
            self.buildIndexConstSlice(HBits(w - shAmount), op0, w - shAmount, 0),  # lower shAmount bits from op0
        )
...
class HlsNetlistBuilderWithWorklist(HlsNetlistBuilder):
    ...
    def __init__(self, builder: HlsNetlistBuilder, worklist: SetList[HlsNetNode]):
        ...
        self.parentElm = builder.parentElm
        self._addNodeOriginal = builder._addNode
        self.operatorCache = builder.operatorCache

  ...
  <tr><td bgcolor="blue">backedge</td></tr>
        ...
        for i, b in  enumerate(mf):
            ...
            if mbSync.useDataAsControl:
                flags.append(f"useDataAsControl={html.escape(repr({f'bb.{bb.getNumber()}': f'r{reg.virtRegIndex()}' for bb, reg in mbSync.useDataAsControl.items()}))}")
            if mbSync.rstPredeccessor:
                flags.append(f"rstPredeccessor=bb.{mbSync.rstPredeccessor.getNumber()}")
            if mbSync.loopGates:
                flags.append(f"loopGates={[g._id for g in mbSync.loopGates]}")
            body = (
                '<table border="0" cellborder="1" cellspacing="0">\n'

...
class HlsSchedulerResourceUseList(List[Dict[object, int]]):
    ...
    def moveUse(self, resourceType, fromClkI: int, toClkI: int):
        ...
        assert _fromClkI >= 0 or _fromClkI >= len(self), ("Moving from usage from clock slot which is not present",
                                                          fromClkI, toClkI, " curRange:", self.clkOffset, self.clkOffset + len(self))
    ...
    def normalizeOffsetTo0(self):
        ...
        if off > 0:
            ...
            for items in islice(self, 0, off):
                assert not items, ("When removing time slot in resouces it must be empty")
            del self[0:off]
            self.clkOffset = 0
        else:
            assert off == 0

...
class InterArchElementNodeSharingAnalysis():
    ...
    :ivar ownerOfNode: a dictionary mapping node to list of :class:`hwtHls.architecture.archElement.ArchElement`
        instance
    :note: if node has multiple owners the owners are using only :class:`hwtHls.netlist.nodes.node.HlsNetNodePartRef`
        instances and ownerOfNode is also using only parts. However the port itself does use original node.
    :ivar ownerOfInput: a dictionary mapping node input to list of :class:`hwtHls.architecture.archElement.ArchElement`
        instance
    :ivar ownerOfOutput: a dictionary mapping node output to :class:`hwtHls.architecture.archElement.ArchElement` instance
    ...
    def _analyzeInterElementsNodeSharingCheckInputDriver(self,
        ...
        if isinstance(dstElm, ArchElementFsm):
            ...
            assert useClkI < len(dstElm.fsm.states) and dstElm.fsm.states[useClkI], (
                useClkI, o, dstElm,
    ...
    def _resolvePortAndNodeOwners(self, archElements: List[ArchElement]):
            ...
            for n in dstElm.allNodes:
                ...
                if isinstance(n, HlsNetNodePartRef):
                    ...
                    if n._subNodes is not None:
                        for subNode in n._subNodes.nodes:
                            for i in subNode._inputs:
                                ...
                                if outerIn is not None:
                                    curOwners = self.ownerOfInput.get(outerIn, None)
                                    if curOwners is None:
                                        self.ownerOfInput[outerIn] = UniqList((dstElm,))
                                    else:
                                        assert isinstance(curOwners, UniqList), curOwners
                                        curOwners.append(dstElm)
                        ...
                        curParentOwner = self.ownerOfNode.get(n, None)
                        assert curParentOwner is None or curParentOwner is dstElm, (n, dstElm, curParentOwner)
                        self.ownerOfNode[n] = dstElm
                else:
                    for i in n._inputs:
                        assert i not in  self.ownerOfInput
    ...
    def _analyzeInterElementsNodeSharing(self, archElements: List[ArchElement]):
        self._resolvePortAndNodeOwners(archElements)
        for dstElm in archElements:
